---
name: "design-tokens"
description: "Дизайн-токены двумя слоями: примитивы и семантика в docs/ds/foundation.md, затем theme.json и тёмная тема, контракт в .claude/rules/wp-theme.md. Применять, когда добавляется цвет, кегль, отступ, размер, радиус или статус каталога, при смене акцента или шрифта; проверка — python scripts/check_tokens.py."
---

# Этап 10 · Дизайн-токены и theme.json

> До 15.09.2026 это была директива `10_directive_tokens.md`; исходник — `_archive/directives_v2/`.
> Этап 10 конвейера (00_pipeline.md). Вход: `docs/brief.md` §8, `docs/brand/brand-direction.md`, grey-box тема из 08. Выход: `docs/ds/foundation.md`, `theme.json`, `assets/css/theme-dark.css`, `assets/fonts/`, обновлённый `.claude/rules/wp-theme.md`, подписи статусов в `docs/VOICE.md`, записи в `docs/DECISIONS.md`. Запуск: /design-tokens

## Задача

Поставить фундамент дизайн-системы сайта: двухслойные токены (примитивы → семантика) сначала текстом в `docs/ds/foundation.md`, затем материализовать их в `theme.json` v3 блочной темы `designstack`, добавить тёмную тему с переключателем и закрепить контракт в `.claude/rules/wp-theme.md`. После этого навыка вся вёрстка (11, 13, 14) берёт цвета, шрифты, отступы только из токенов.
...
- `docs/brand/brand-direction.md` (акцент, шрифт, характер) — из 09. Нет → `/brand`; если заказчик хочет идти без бренда — взять значения из brief §8 и допущения A7, записать в `foundation.md` как «A-10.x, пересмотреть после 09».
- Тема `wordpress/wp-content/themes/designstack/` существует (grey-box из 08: `style.css`, `theme.json`, `functions.php`, `templates/index.html`). Нет → `/wireframes`.
...
Дальше: /patterns-library — паттерны и витрина /styleguide/
```
...
- Без фазы 5 навык не завершён: без контракта фундамент — декорация.
