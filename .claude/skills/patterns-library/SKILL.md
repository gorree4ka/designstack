...
description: "Библиотека паттернов темы и витрина /styleguide/: каждый паттерн во всех вариантах и состояниях, только на токенах (docs/ds/components.md, patterns/*.php). Применять, когда нужен новый паттерн или состояние либо разметка повторилась трижды."
---

# Этап 11 · Библиотека паттернов и витрина

> До 15.09.2026 это была директива `11_directive_patterns.md`; исходник — `_archive/directives_v2/`.
> Этап 11 конвейера (00_pipeline.md). Вход: `docs/ds/foundation.md`, `theme.json`, `docs/ia/wireframes/*.md`. Выход: `docs/ds/components.md`, `patterns/*.php`, `parts/*.html`, `assets/css/patterns.css`, витрина `/styleguide/`. Запуск: /patterns-library
...
- `docs/ds/foundation.md`, `theme.json` с семантической палитрой, `assets/css/theme-dark.css` — из 10 (иначе `/design-tokens`).
...
Дальше: /core-plugin (плагин и данные), затем /page-templates (сборка страниц)
```
