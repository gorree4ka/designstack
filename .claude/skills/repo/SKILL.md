---
name: "repo"
description: "Репозиторий GitHub: что попадает в git, коммит и пуш проверенного состояния, README.md и PROJECT.md. Применять, когда заказчик просит сохранить работу в репозиторий или подготовить код к деплою."
---

# Этап 18 · Репозиторий GitHub

> До 15.09.2026 это была директива `18_directive_repo.md`; исходник — `_archive/directives_v2/`.
> Этап 18 конвейера (00_pipeline.md). Вход: тема `wordpress/wp-content/themes/designstack/`, плагин `wordpress/wp-content/plugins/designstack-core/`, `scripts/`, `docs/`, `.gitignore`. Выход: приватный репозиторий на GitHub (ветка `main`), `README.md`, `PROJECT.md`. Запуск: `/repo`
...
- Если нет темы или плагина — сначала `11`–`14`; навык не заливает пустой каркас.
...
1. `README.md` (русский, ≤80 строк): что это (одна фраза из brief §2 + позиционирование), стек (WordPress 7.x, PHP 8.3, блочная тема `designstack`, плагин `designstack-core`, локально SQLite), как поднять локально — `python scripts/setup.py` → `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/start.ps1 -Background` → `python scripts/seed.py` → http://localhost:8080 (логин в `.env`), структура папок (тема, плагин, `scripts/`, `docs/`, `.claude/`), конвейер — ссылка на `00_pipeline.md`, как работать с агентом (навыки в `.claude/skills/` поднимаются по описанию или вызываются по имени). Без секретов, без внутренних адресов хостинга. Скелет:
   ```markdown
   ...
   ## Структура       wordpress/wp-content/themes/designstack/ · plugins/designstack-core/ · scripts/ · docs/ · .claude/skills/
   ## Конвейер        00_pipeline.md — этапы 02–19, каждый этап живёт навыком в .claude/skills/; требования — docs/brief.md
...
- Есть тема, плагин, `scripts/`, `docs/`, `.claude/` с навыками и правилами, `README.md`, `PROJECT.md`.
...
- Залито одним пушем: тема designstack, плагин designstack-core, scripts/, docs/, .claude/, README, PROJECT.md (<N> файлов)
...
Следующий шаг: /deploy — деплой на хостинг (или /seo-analytics для раунда метрик).
```
...
- Репозиторий и токен создаёт заказчик по шагам навыка; агент ведёт командную строку.
- Токен транзиентный: Credential Manager или одна команда; не в файлы, не в конфиг, не в отчёт.
- В git — только своё: тема, плагин, скрипты, документы, настройки агента. Ядро, инструменты, база, загрузки, секреты — нет.
- Коммиты с атрибуцией `Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>`; коммитить только по запросу заказчика или в рамках этого навыка.
