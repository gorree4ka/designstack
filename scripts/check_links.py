...
Навык `catalog-content`, фаза 6. Скрипт **ничего не меняет** в WordPress: он собирает факты и
...
def write_report(report: list) -> None:
    ...
    lines = [
        ...
        "Доступ и оплата из России здесь не проверяются — это три пробы фазы 1.5 навыка `catalog-content`.",
        "",
        "## Требуют решения",
        "",
    ]
